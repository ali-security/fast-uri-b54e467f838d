...
function parseWithStatus (uri, opts) {
  ...
  if (matches) {
    ...
    // Percent-encoding in the scheme is unescaped below, so the decoded value
    // has to be re-validated against the RFC 3986 "scheme" rule before it is
    // used. Otherwise "%2f%2fevil.example:/pwn" decodes to "//evil.example",
    // and re-serializing the components injects an authority (or raw CRLF for
    // "%0d%0a...") that the original input never contained.
    ...
    if (!options.unicodeSupport && (!schemeHandler || !schemeHandler.unicodeSupport)) {
      ...
      // A bracketed IP literal is never a domain name: it has already been
      // validated as an IP literal above, so it must not be handed to the
      // WHATWG domain converter, which would silently rewrite a malformed
      // literal into some other host. An unterminated bracket is not a literal
      // and still goes through the conversion, which rejects it.
...
function normalizeComparableURI (uri, opts) {
  ...
  if (typeof uri === 'object') {
    // A component object with a scheme that does not survive RFC 3986
    // re-validation cannot be compared, so equality fails closed instead of
    // propagating the serialization error.
    try {
